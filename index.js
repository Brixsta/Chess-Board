...
class ChessBoard {
  ...
  #highlightTiles(item, e) {
    ...
    if (game.activeTile === e.path[1]) {
      ...
      return;
    } else if (item && item.children.length > 0 && game.activeTile === null) {
      // no prior active piece
      let pieceColor = e.path[1].children[0].src.includes("white");
      pieceColor
        ? (game.activePieceColor = true)
        : (game.activePieceColor = false);
    ...
    } else if (item && item.children.length > 0 && game.activeTile) {
      ...
      item.addEventListener("click", (e) => {
        ...
        if (
          ...
          if (
            game.activeTile &&
            game.previousPieceColor !== null &&
            game.activePieceColor !== game.previousPieceColor
          ) {
